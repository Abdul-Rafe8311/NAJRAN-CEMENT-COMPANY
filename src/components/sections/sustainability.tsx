...
export function Sustainability() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{ y: yBlob }}
        className="pointer-events-none absolute right-[-10%] top-1/4 h-[50vh] w-[50vh] rounded-full bg-[#1f7a5a]/20 blur-[130px]"
      />
      ...
      <div className="container-page relative grid items-start gap-14 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-10 space-y-px overflow-hidden rounded-[var(--radius-card)] border border-line">
            {SUSTAIN_POINTS.map((point, i) => (
              <Reveal key={i} delay={i * 0.08} className="flex items-start gap-4 bg-coal p-5">
                <span className="mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#1f7a5a]/15 text-[#1f7a5a]">
                ...
                <p className="text-sm leading-relaxed text-ash">{point}</p>
              </Reveal>
            ))}
          </div>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-1 lg:mt-24">
          {SUSTAIN_METRICS.map((m, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="rounded-[var(--radius-card)] border border-line bg-coal p-7">
                <div className="flex items-center justify-between">
                  <span className="font-display text-5xl font-semibold text-[#1f7a5a]">
                    {m.value > 0 ? `${m.value}${m.suffix ?? ""}` : "—"}
                  </span>
                  {m.review && <ReviewBadge />}
                </div>
                <p className="mt-4 text-sm leading-relaxed text-ash">{m.label}</p>
              </div>
            </Reveal>
          ))}
        </div>
